...
{
	bFireLeft = false;
    EHAssaultGrenade(FireMode[1]).ReturnToIdle();
...
function AttachToPawn(Pawn P)
{
	...
	if ( bDualMode )
	{
		...
		if ( OffhandActor == None )
		{
			OffhandActor = EHAssaultAttachment(Spawn(AttachmentClass,Owner));
		...
		else
		{
			...
			OffhandActor.TwinGun = EHAssaultAttachment(ThirdPersonActor);
			if ( Mesh == OldMesh )
			{
			...
			EHAssaultAttachment(ThirdPersonActor).TwinGun = OffhandActor;
		}
	}
}
...
{
	return FMin(1,FireMode[1].HoldTime/EHAssaultGrenade(FireMode[1]).mHoldClampMax);
}
...
defaultproperties
{
     FireModeClass(0)=Class'mm_EHWeapons.EHAssaultFire'
     FireModeClass(1)=Class'mm_EHWeapons.EHAssaultGrenade'
     PutDownAnim="PutDown"
     ...
     InventoryGroup=2
     PickupClass=Class'mm_EHWeapons.EHAssaultRiflePickup'
     PlayerViewOffset=(X=4.000000,Y=5.500000,Z=-6.000000)
     PlayerViewPivot=(Pitch=400)
     BobDamping=1.700000
     AttachmentClass=Class'mm_EHWeapons.EHAssaultAttachment'
