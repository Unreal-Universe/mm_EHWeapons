...
var() class<EHIonEffect> EHIonEffectClass;
...
function PostBeginPlay()
{
	...
	ForEach DynamicActors(Class'EHIonCannon',C)
	{
		if ( C != self )
			C.Destroy();
	}
...
state FireSequence
{
    ...
    {
        local EHIonEffect EHIonBeamEffect;
        local Actor Other;
        ...
        EHIonBeamEffect = Spawn(EHIonEffectClass,,, Top);
        if (EHIonBeamEffect != None)
            EHIonBeamEffect.AimAt(MarkLocation, Vect(0,0,1));
        ...
        if ( Other == None )
        {
			EHIonBeamEffect = Spawn(EHIonEffectClass,,, Top + CP);
			if (EHIonBeamEffect != None)
				EHIonBeamEffect.AimAt(MarkLocation, Vect(0,0,1));
		}
...
Begin:
    ...
    Sleep(0.5);
	PlayGlobalSound(Sound'mm_EHWeapons.EHSounds.EMP_strike_impact');
...
defaultproperties
{
     FireSound=Sound'mm_EHWeapons.EHSounds.EMP_strike_impact'
     EHIonEffectClass=Class'mm_EHWeapons.EHIonEffect'
     Damage=150
     MomentumTransfer=150000.000000
     DamageRadius=2000.000000
     DamageType=Class'mm_EHWeapons.DamTypeEHIonBlast'
