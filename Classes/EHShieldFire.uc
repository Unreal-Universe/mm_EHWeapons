...
var xEmitter EHChargingEmitter;           // emitter class while charging
...
{
    if (EHChargingEmitter != None)
        EHChargingEmitter.Destroy();
...
simulated function InitEffects()
{
	...
	{
		EHChargingEmitter = Weapon.Spawn(class'EHWeaponsE.EHShieldCharge');
...
{
    if (EHChargingEmitter != None && HoldTime > 0.0 && !bNowWaiting)
    {
		EHChargingEmitter.SetLocation( Weapon.GetEffectStart() );
        Canvas.DrawActor( EHChargingEmitter, false, false, Weapon.DisplayFOV );
    }
...
function Timer()
{
    ...
    if (HoldTime > 0.0 && !bNowWaiting)
    {
        ...
        else
        {
            ...
            if (EHChargingEmitter != None)
            {
                EHChargingEmitter.mRegenPause = false;
                Regen = ChargeScale * 10 + 20;
                EHChargingEmitter.mRegenRange[0] = Regen;
                EHChargingEmitter.mRegenRange[1] = Regen;
                EHChargingEmitter.mSpeedRange[0] = ChargeScale * -15.0;
                EHChargingEmitter.mSpeedRange[1] = ChargeScale * -15.0;
                Regen = FMax((ChargeScale / 30.0),0.20);
                EHChargingEmitter.mLifeRange[0] = Regen;
                EHChargingEmitter.mLifeRange[1] = Regen;
            }
...
defaultproperties
{
     DamageType=Class'EHWeaponsE.DamTypeEHShieldImpact'
     ...
     ChargingSound=Sound'ONSVehicleSounds-S.PowerCore.CoreHvyDmg01'
     ...
     FlashEmitterClass=Class'EHWeaponsE.EHForceRingA'
}
