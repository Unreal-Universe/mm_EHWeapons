...
{
	EHRocketLauncher(Weapon).PlayOwnedSound(Sound'WeaponSounds.RocketLauncher.RocketLauncherLoad', SLOT_None,,,,,false);
}
...
{
    EHRocketLauncher(Weapon).PlayLoad(false);
}
...
function PlayFiring()
{
    if (Load > 1.0)
        ...
        FireAnim = 'Fire';
    Super.PlayFiring();
    EHRocketLauncher(Weapon).PlayFiring((Load == MaxLoad));
	Weapon.OutOfAmmo();
}

event ModeDoFire()
{
    if ( EHRocketLauncher(Weapon).bTightSpread || ((Bot(Instigator.Controller) != None) && (FRand() < 0.65)) )
    {
        Spread = TightSpread;
		SpreadStyle = SS_Ring;
    }
    ...
    EHRocketLauncher(Weapon).bTightSpread = false;
    Super.ModeDoFire();
...
function DoFireEffect()
{
	...
	local EHRocketProj FiredRockets[4];
	local bool bCurl;
    ...
    for ( p=0; p<SpawnCount; p++ )
    {
        ...
        FiredRockets[p] = EHRocketProj(SpawnProjectile(FireLocation, Aim));
    }
