...
simulated function bool myHasAmmo( EHGrappleGun EHGrappleGun )
{
	return (EHGrappleGun.AmmoAmount(ThisModeNum) >= AmmoPerFire);
}
...
simulated function float AdjustLinkDamage( EHGrappleGun EHGrappleGun, Actor Other, float Damage )
{
	return Damage * (1.5*EHGrapplegun.Links+1);
}
...
simulated function ModeTick(float dt)
{
	...
	local Actor Other;
	local Rotator Aim;
	local EHGrappleGun EHGrappleGun;
	local float Step, ls;
	local bot B;
	local bool bShouldStop, bIsHealingObjective;
	local int AdjustedDamage;
	local EHGrappleBeamEffect LB;
    ...
    EHGrappleGun = EHGrappleGun(Weapon);

    if ( EHGrappleGun.Links < 0 )
    {
        log("warning:"@Instigator@"EHGrapplegun had"@EHGrappleGun.Links@"links");
        EHGrappleGun.Links = 0;
    }

    ls = LinkScale[Min(EHGrappleGun.Links,5)];

    if ( myHasAmmo(EHGrappleGun) && ((UpTime > 0.0) || (Instigator.Role < ROLE_Authority)) )
    {
        UpTime -= dt;

		// the to-hit trace always starts right in front of the eye
		EHGrappleGun.GetViewAxes(X, Y, Z);
		StartTrace = GetFireStart( X, Y, Z);
        TraceRange = default.TraceRange + EHGrappleGun.Links*250;

        if ( Instigator.Role < ROLE_Authority )
        {
			if ( Beam == None )
				foreach Weapon.DynamicActors(class'EHGrappleBeamEffect', LB )
		...
		{
		    if ( bDoHit )
			    EHGrappleGun.ConsumeAmmo(ThisModeNum, AmmoPerFire);
		...
		if ( Instigator.Role < ROLE_Authority )
		{
			if ( EHGrappleGun.ThirdPersonActor != None )
			{
				if ( EHGrappleGun.Linking || ((Other != None) && (Instigator.PlayerReplicationInfo.Team != None) && Other.TeamLink(Instigator.PlayerReplicationInfo.Team.TeamIndex)) )
				{
					if (Instigator.PlayerReplicationInfo.Team == None || Instigator.PlayerReplicationInfo.Team.TeamIndex == 0)
						EHGrappleAttachment(EHGrappleGun.ThirdPersonActor).SetLinkColor( LC_Red );
					else
						EHGrappleAttachment(EHGrappleGun.ThirdPersonActor).SetLinkColor( LC_Blue );
				}
				else
				{
					if ( EHGrappleGun.Links > 0 )
						EHGrappleAttachment(EHGrappleGun.ThirdPersonActor).SetLinkColor( LC_Gold );
					else
						EHGrappleAttachment(EHGrappleGun.ThirdPersonActor).SetLinkColor( LC_Green );
				}
			}
			return;
		}
        if ( Other != None && Other != Instigator )
        {
            ...
            else
            {
                ...
                if ( bDoHit )
                {
                    ...
                    AdjustedDamage = AdjustLinkDamage( EHGrappleGun, Other, Damage );
		...
		{
			AdjustedDamage = Damage * (1.5*EHGrapplegun.Links+1) * Instigator.DamageScaling;
			if (Instigator.HasUDamage())
				AdjustedDamage *= 2;
			if (!LinkedVehicle.HealDamage(AdjustedDamage, Instigator.Controller, DamageType))
				EHGrappleGun.ConsumeAmmo(ThisModeNum, -AmmoPerFire);
		}
		EHGrappleGun(Weapon).Linking = (LockedPawn != None) || bIsHealingObjective;
...
function PlayFiring()
{
	if (EHGrappleGun(Weapon).Links <= 0 && Weapon.AmmoAmount(ThisModeNum) >= AmmoPerFire)
...
function StopFiring()
{
    ...
    if (EHGrappleGun(Weapon).Links <= 0)
...
function SetLinkTo(Pawn Other)
{
    ...
    {
        RemoveLink(1 + EHGrappleGun(Weapon).Links, Instigator);
        EHGrappleGun(Weapon).Linking = false;
    }
    ...
    if (LockedPawn != None)
    {
        if (!AddLink(1 + EHGrappleGun(Weapon).Links, Instigator))
        {
            bFeedbackDeath = true;
        }
        EHGrappleGun(Weapon).Linking = true;
...
function bool AddLink(int Size, Pawn Starter)
{
    ...
    if (LockedPawn != None && !bFeedbackDeath)
    {
        ...
        {
            Inv = LockedPawn.FindInventoryType(class'EHGrappleGun');
            if (Inv != None)
            {
                if (EHGrappleFire(EHGrappleGun(Inv).GetFireMode(1)).AddLink(Size, Starter))
                    EHGrappleGun(Inv).Links += Size;
                else
                    return false;
            }
        }
...
function RemoveLink(int Size, Pawn Starter)
{
    ...
    if (LockedPawn != None && !bFeedbackDeath)
    {
        if (LockedPawn != Starter)
        {
            ...
            if (Inv != None)
            {
                EHGrappleFire(EHGrappleGun(Inv).GetFireMode(1)).RemoveLink(Size, Starter);
                EHGrappleGun(Inv).Links -= Size;
            }
        }
    }
...
function bool IsLinkable(Actor Other)
{
    ...
    {
        P = Pawn(Other);
        if ( P.Weapon == None || !P.Weapon.IsA('EHGrappleGun') )
		{
			if ( Vehicle(P) != None )
        ...
        LG = EHGrappleGun(P.Weapon);
        LF = EHGrappleFire(LG.GetFireMode(1));
        while ( LF != None && LF.LockedPawn != None && LF.LockedPawn != P && sanity < 32 )
        {
            if ( LF.LockedPawn == Instigator )
                return false;

            LG = EHGrappleGun(LF.LockedPawn.Weapon);
            if ( LG == None )
                break;
            LF = EHGrappleFire(LG.GetFireMode(1));
            sanity++;
...
defaultproperties
{
     BeamEffectClass=Class'EHWeaponsE.EHGrappleBeamEffect'
     ...
     DamageType=Class'EHWeaponsE.DamTypeEHGrappleShaft'
     ...
     FireRate=0.120000
     AmmoClass=Class'EHWeaponsE.EHGrappleAmmo'
