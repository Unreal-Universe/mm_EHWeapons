...
var EHLinkMuzFlashBeam3rd MuzFlash, OldMuzFlash;

const MAX_CHILDREN = 4;
var int				NumChildren;
var EHLinkBeamChild	Child[MAX_CHILDREN];
...
simulated function Tick(float dt)
{
	...
	if ( Level.NetMode != NM_DedicatedServer )
	{
		...
		{
			if ( MuzFlash == None )
				MuzFlash = Spawn(class'EHLinkMuzFlashBeam3rd', self);
		}
    ...
    ls = class'EHLinkFire'.default.LinkScale[Min(Links,5)];
    ...
    if ( Links != OldLinks || LinkColor != OldLinkColor || MuzFlash != OldMuzFlash )
    {
		...
		if ( Level.NetMode != NM_DedicatedServer )
		{
			for (c=0; c<MAX_CHILDREN; c++)
			{
				...
				{
					if ( Child[c] == None )
						Child[c] = Spawn(class'EHLinkBeamChild', self);
        ...
        if ( LinkColor == 0 )
        {
            if ( Links > 0 )
            {
                Skins[0] = FinalBlend'EHWeaponSkinsB.LinkBeamYellowFB';
                if ( MuzFlash != None )
					MuzFlash.Skins[0] = Texture'EHWeaponSkinsB.LinkMuzYellow';
                LightHue = 40;
            }
            else
            {
                ...
                if ( MuzFlash != None )
	                MuzFlash.Skins[0] = Texture'EHWeaponSkinsB.LinkMuzGreen';
                LightHue = 100;
            }
        }
        else if ( LinkColor == 1 )
        {
            Skins[0] = FinalBlend'EHWeaponSkinsB.LinkBeamRedFB';
            if ( MuzFlash != None )
				MuzFlash.Skins[0] = Texture'EHWeaponSkinsB.LinkMuzRed';
            LightHue = 0;
        }
        else
        {
            Skins[0] = FinalBlend'EHWeaponSkinsB.LinkBeamBlueFB';
            if ( MuzFlash != None )
	            MuzFlash.Skins[0] = Texture'EHWeaponSkinsB.LinkMuzBlue';
            LightHue = 160;
        }
        ...
        if ( LinkColor > 0 && LinkedPawn != None )
        {
            if ( (ProtSphere == None) && (Level.NetMode != NM_DedicatedServer) )
            {
                ...
                if (LinkColor == 2)
                    ProtSphere.Skins[0] = Texture'EHWeaponSkinsB.LinkMuzBlue';
            }
        }
    ...
    {
        mWaveAmplitude = FMax(4.0, mWaveAmplitude - (mWaveAmplitude+5)*4.0*dt);
    }
    else
    {
    ...
    if ( bHitSomething && (Level.NetMode != NM_DedicatedServer) && (Level.TimeSeconds - ScorchTime > 0.07) )
    {
		...
		if ( (HitActor != None) && HitActor.bWorldGeometry )
			spawn(class'EHLinkScorch',,,HitLocation,rotator(-HitNormal));
			spawn(class'HitSmokeBlack',,,HitLocation,rotator(-HitNormal));
...
defaultproperties
{
     EffectOffset=(X=68.000000,Y=40.000000,Z=1.400000)
     mParticleType=PT_Beam
     mMaxParticles=6
     mRegenDist=6.000000
     mSpinRange(0)=15000.000000
