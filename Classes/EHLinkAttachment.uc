class EHLinkAttachment extends xWeaponAttachment;
...
simulated function UpdateLinkColor()
{
	if ( Instigator != None && Instigator.Weapon != None )
		EHLinkGun(Instigator.Weapon).UpdateLinkColor( LinkColor );
...
simulated event ThirdPersonEffects()
{
    ...
    if ( Level.NetMode != NM_DedicatedServer && FlashCount > 0 )
	{
        if (FiringMode == 0)
        {
            ...
            if (MuzFlash != None)
            {
                MuzFlash.mSizeRange[0] = MuzFlash.default.mSizeRange[0] * (class'EHLinkFire'.default.LinkScale[Min(Links,5)]+1); // (1.0 + 0.3*float(Links));
