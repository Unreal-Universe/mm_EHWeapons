class EHMinigunAttachment extends xWeaponAttachment;
		...
		if ( OldSpawnHitCount != SpawnHitCount )
		{
			...
			if ( (Instigator.Controller == PC) || (VSize(PC.ViewTarget.Location - mHitLocation) < 2000) )
			{
				if ( FiringMode == 0 )
					...
					Spawn(class'EHWeaponsE.EHExploWallHit',,, mHitLocation, Rotator(mHitNormal));
				if ( FiringMode == 1 )
...
defaultproperties
{
     ...
     mTracerClass=Class'EHWeaponsE.EHMinigunTracer'
