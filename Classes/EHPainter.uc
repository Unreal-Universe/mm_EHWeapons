...
simulated event RenderOverlays( Canvas Canvas )
{
    ...
    local EHPainterFire EHPainterFire;
    ...
    {
		EHPainterFire = EHPainterFire(FireMode[0]);
        if (EHPainterFire.bIsFiring && EHPainterFire.bValidMark && Level.TimeSeconds - EHPainterFire.MarkTime > 0.4)
        {
            ChargeBar = FMin(1.0, ((Level.TimeSeconds - EHPainterFire.MarkTime) / EHPainterFire.PaintDuration));
...
function EHIonCannon CheckMark(vector MarkLocation, bool bFire)
{
	if ( FirstCannon != None )
		return FirstCannon;
	foreach DynamicActors(class'EHIonCannon', FirstCannon)
		return FirstCannon;
	FirstCannon = spawn(class'EHIonCannon',,,Location+vect(0,0,2000));
	return FirstCannon;
}
...
defaultproperties
{
     ...
     FireModeClass(0)=Class'mm_EHWeapons.EHPainterFire'
     ...
     InventoryGroup=0
     PickupClass=Class'mm_EHWeapons.EHPainterPickup'
     PlayerViewOffset=(X=25.000000,Y=2.000000,Z=-1.000000)
     BobDamping=1.575000
     AttachmentClass=Class'mm_EHWeapons.EHPainterAttachment'
