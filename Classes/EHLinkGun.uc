...
// EHLink Gun
//=============================================================================
class EHLinkGun extends Weapon
    config(user);
...
{
	if ( FireMode[1] != None )
		EHLinkFire(FireMode[1]).UpdateLinkColor( color );
	...
	switch ( Color )
	{
		case LC_Green	:	Skins[0] = material'LinkGunTex02';
							Skins[1] = material'PowerPulseShader';
							break;
		case LC_Red		: 	Skins[0] = material'LinkGunTex02';
							Skins[1] = material'PowerPulseShaderRed';
							break;
		case LC_Blue	: 	Skins[0] = material'LinkGunTex02';
							Skins[1] = material'PowerPulseShaderBlue';
							break;
		case LC_Gold	:	Skins[0] = material'LinkGunTex02';
...
simulated event RenderOverlays( Canvas Canvas )
{
	...
	{
		if ( Links > 0 )
			EHLinkAttachment(ThirdPersonActor).SetLinkColor( LC_Gold );
		else
			EHLinkAttachment(ThirdPersonActor).SetLinkColor( LC_Green );
	}
...
simulated function Destroyed()
{
    ...
    {
		if ( EHLinkFire(FireMode[1]) != None )
			EHLinkFire(FireMode[1]).SetLinkTo(None);
    }
    Super.Destroyed();
}
...
function bool FocusOnLeader(bool bLeaderFiring)
{
	...
	if ( (Vehicle(LeaderPawn) != None)
		|| ((EHLinkGun(LeaderPawn.Weapon) != None) && ((vector(B.Squad.SquadLeader.Rotation) dot Normal(Instigator.Location - LeaderPawn.Location)) < 0.9)) )
	{
		StartTrace = Instigator.Location + Instigator.EyePosition();
		if ( VSize(LeaderPawn.Location - StartTrace) < EHLinkFire(FireMode[1]).TraceRange )
		{
...
function float GetAIRating()
{
	...
	if ( (PlayerController(B.Squad.SquadLeader) != None)
		...
		&& (EHLinkGun(B.Squad.SquadLeader.Pawn.Weapon) != None) )
		return 1.2;

	V = B.Squad.GetLinkVehicle(B);
	if ( (V != None)
		&& (VSize(Instigator.Location - V.Location) < 1.5 * EHLinkFire(FireMode[1]).TraceRange)
		&& (V.Health < V.HealthMax) && (V.LinkHealMult > 0) )
		return 1.2;

	if ( Vehicle(B.RouteGoal) != None && B.Enemy == None && VSize(Instigator.Location - B.RouteGoal.Location) < 1.5 * EHLinkFire(FireMode[1]).TraceRange
	...
	if ( O != None && B.Enemy == None && O.TeamLink(B.GetTeamNum()) && O.Health < O.DamageCapacity
	     && VSize(Instigator.Location - O.Location) < 1.1 * EHLinkFire(FireMode[1]).TraceRange && B.LineOfSightTo(O) )
		return 1.2;
...
function byte BestMode()
{
	...
	if ( V == B.Target )
		return 1;
	if ( (V != None) && (VSize(Instigator.Location - V.Location) < EHLinkFire(FireMode[1]).TraceRange)
		&& (V.Health < V.HealthMax) && (V.LinkHealMult > 0) && B.LineOfSightTo(V) )
		return 1;
	if ( B.Enemy == None )
		return 0;
	EnemyDist = VSize(B.Enemy.Location - Instigator.Location);
	if ( EnemyDist > EHLinkFire(FireMode[1]).TraceRange )
		return 0;
	return 1;
}
...
function bool LinkedTo(EHLinkGun L)
{
	local Pawn Other;
	local EHLinkGun OtherWeapon, Head;
	local int sanity;

	Head = self;
	while (Head != None && Head.Linking && sanity < 20)
	{
            Other = EHLinkFire(Head.FireMode[1]).LockedPawn;
            if (Other == None)
                return false;
            else
            {
                OtherWeapon = EHLinkGun(Other.Weapon);
                if (OtherWeapon == None)
                    return false;
                else
                    Head = OtherWeapon;
...
function bool ConsumeAmmo(int Mode, float load, optional bool bAmountNeededIsMax)
{
	...
	if (Linking && EHLinkFire(FireMode[1]).LockedPawn != None && Vehicle(EHLinkFire(FireMode[1]).LockedPawn) == None)
		return true;
	...
	if (Instigator != None && Instigator.PlayerReplicationInfo != None && Instigator.PlayerReplicationInfo.Team != None)
	{
		for (C = Level.ControllerList; C != None; C = C.NextController)
			if (C.Pawn != None && EHLinkGun(C.Pawn.Weapon) != None && EHLinkGun(C.Pawn.Weapon).LinkedTo(self))
				EHLinkGun(C.Pawn.Weapon).LinkedConsumeAmmo(Mode, load, bAmountNeededIsMax);
	}
...
simulated function IncrementFlashCount(int mode)
{
	...
	if ( EHLinkAttachment(ThirdPersonActor) != None )
        EHLinkAttachment(ThirdPersonActor).Links = Links;
}
...
defaultproperties
{
     FireModeClass(0)=Class'EHWeaponsE.EHLinkAltFire'
     FireModeClass(1)=Class'EHWeaponsE.EHLinkFire'
     PutDownAnim="PutDown"
     IdleAnimRate=0.030000
     SelectSound=Sound'NewWeaponSounds.NewLinkSelect'
     SelectForce="SwitchToEHLinkGun"
     AIRating=0.680000
     ...
     InventoryGroup=5
     PickupClass=Class'EHWeaponsE.EHLinkGunPickup'
     PlayerViewOffset=(X=-5.000000,Y=-3.000000)
     PlayerViewPivot=(Yaw=500)
     BobDamping=1.575000
     AttachmentClass=Class'EHWeaponsE.EHLinkAttachment'
