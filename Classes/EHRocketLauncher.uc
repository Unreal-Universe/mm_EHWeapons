...
{
    local EHRocketProj Rocket;
    local SeekingEHRocketProj SeekingRocket;
	local bot B;
    ...
    {
        SeekingRocket = Spawn(class'SeekingEHRocketProj',,, Start, Dir);
    ...
    {
        Rocket = Spawn(class'EHRocketProj',,, Start, Dir);
        return Rocket;
    }
}
...
simulated event ClientStartFire(int Mode)
{
	...
	if ( EHRocketMultiFire(FireMode[Mode]) != None )
	{
...
Begin:
    ...
    RotateBarrel();
    Sleep(0.07);
    PlayOwnedSound(Sound'CicadaSnds.Missile.MissileLoad', SLOT_None,,,,,false);
    ClientPlayForceFeedback( "RocketLauncherLoad" );  // jdf
    Sleep(0.28);
    Plunge();
    PlayOwnedSound(Sound'WeaponSounds.BaseGunTech.BReload2', SLOT_None,,,,,false);
...
defaultproperties
{
     ...
     FireModeClass(0)=Class'mm_EHWeapons.EHRocketFire'
     FireModeClass(1)=Class'mm_EHWeapons.EHRocketMultiFire'
     SelectAnim="Pickup"
     PutDownAnim="PutDown"
     IdleAnimRate=0.500000
     SelectSound=Sound'WeaponSounds.RocketLauncher.SwitchToRocketLauncher'
     SelectForce="SwitchToEHRocketLauncher"
     AIRating=0.780000
     ...
     InventoryGroup=8
     PickupClass=Class'mm_EHWeapons.EHRocketLauncherPickup'
     PlayerViewOffset=(Y=8.000000)
     PlayerViewPivot=(Yaw=500,Roll=1000)
     BobDamping=1.500000
     AttachmentClass=Class'mm_EHWeapons.EHRocketAttachment'
