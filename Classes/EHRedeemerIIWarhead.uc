...
#exec OBJ LOAD FILE="Resources\tk_EHWeapons_rc.u" PACKAGE="mm_EHWeapons" PACKAGE="mm_EHWeapons"
...
simulated function PostBeginPlay()
{
	...
	{
		SmokeTrail = Spawn(class'EHRedeemerIISmokeTrail',self,,Location - 40 * Dir);
...
simulated function PostNetBeginPlay()
{
	...
	if ( PlayerController(Controller) != None )
	{
		...
		PlayOwnedSound(Sound'WeaponSounds.BaseImpactAndExplosions.BExplosion4',SLOT_Interact,1.0);
	}
}
...
function BlowUp(vector HitLocation)
{
	...
	{
		bHidden = true;
        E = Spawn(Class'NewExplosionA',,, HitLocation - 100 * Normal(Velocity), Rot(0,16384,0));
		if ( Level.NetMode == NM_DedicatedServer )
		{
			E.LifeSpan = 0.7;
		}
...
Begin:
	Instigator = self;
    PlaySound(Sound'mm_EHWeapons.EHSounds.RIIExplode',SLOT_None,5*TransientSoundVolume);
...
defaultproperties
{
     ...
     MyDamageType=Class'mm_EHWeapons.DamTypeEHRedeemerII'
     ...
     AmbientSound=Sound'mm_EHWeapons.EHSounds.EHDeemerFlight'
     DrawScale=0.500000
     AmbientGlow=96
     bGameRelevant=True
     bCanTeleport=False
