...
function DoFireEffect()
{
    ...
    if ( EHAssaultRifle(Weapon).bDualMode )
    {
		EHAssaultRifle(Weapon).bFireLeft = !EHAssaultRifle(Weapon).bFireLeft;
		if ( EHAssaultRifle(Weapon).bFireLeft )
			Y *= -1;
	}
...
function projectile SpawnProjectile(Vector Start, Rotator Dir)
{
    ...
    g = Weapon.Spawn(class'EHGrenade', instigator,, Start, Dir);
    if (g != None)
    {
        Weapon.GetViewAxes(X,Y,Z);
