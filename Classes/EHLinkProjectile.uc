...
simulated function PostNetBeginPlay()
{
	...
	if ( (Level.NetMode != NM_DedicatedServer) && (Level.DetailMode != DM_Low) )
		Trail = Spawn(class'NewEHLinkTrail',self);
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
    if ( EffectIsRelevant(Location,false) )
	{
        ...
        if (Links == 0)
            Spawn(class'GoopSmoke',,, HitLocation, rotator(HitNormal));
        else
...
defaultproperties
{
     ...
     bDynamicLight=True
     AmbientSound=Sound'GeneralAmbience.electricalfx14'
     LifeSpan=3.000000
     DrawScale3D=(X=3.295000,Y=0.530000,Z=0.530000)
