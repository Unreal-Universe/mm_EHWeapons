...
var class<EHTransTrail> EHTransTrailClass;
var class<EHTransFlareBlue> EHTransFlareClass;
...
simulated function PostBeginPlay()
{
    ...
    Trail = Spawn(EHTransTrailClass, self,, Location, Rotation);
    SetTimer(0.3,false);
}
...
simulated function HitWall( vector HitNormal, actor Wall )
{
	...
	if ( Speed < 20 && Wall.bWorldGeometry && (HitNormal.Z >= 0.7) )
	{
		...
		{
			Flare = Spawn(EHTransFlareClass, self,, Location - vect(0,0,5), rot(16384,0,0));
			Flare.SetBase(self);
		}
	}
}
...
State MonitoringThrow
{
	...
	function BotTranslocate()
	{
		if ( EHTransLauncher(Instigator.Weapon) != None )
...
defaultproperties
{
     ...
     EHTransTrailClass=Class'mm_EHWeapons.EHTransTrail'
     EHTransFlareClass=Class'mm_EHWeapons.EHTransFlareRed'
     Speed=1200.000000
     DamageRadius=100.000000
     MomentumTransfer=50000.000000
     MyDamageType=Class'mm_EHWeapons.DamTypeEHTeleFrag'
     ...
     Skins(0)=Texture'mm_EHWeapons.EHWeaponSkinsB.EHNEWTranslocatorPUCK'
     AmbientGlow=12
     bUnlit=False
     bShadowCast=True
