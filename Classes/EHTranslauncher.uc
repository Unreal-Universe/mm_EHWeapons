...
function bool ShouldTranslocatorHop(Bot B)
{
	...
	{
		if ( (EHTransBeacon != None) && EHTransBeacon.IsMonitoring(B.Focus) )
			return false;
...
function FireHack(byte Mode)
{
	...
	if ( Mode == 0 )
	{
		if ( EHTransBeacon != None )
		{
			// this shouldn't happen
			EHTransBeacon.bNoAI = true;
		...
		if ( JumpSpot(TTarget) != None )
		{
			...
			if ( (Instigator.Anchor != None) && Instigator.ReachedDestination(Instigator.Anchor) )
			{
				...
				EHTransbeacon.SetLocation(EHTransBeacon.Location + Instigator.Anchor.Location + (Instigator.CollisionHeight - Instigator.Anchor.CollisionHeight) * vect(0,0,1)- Instigator.Location);
			}
		}
		...
		EHTransBeacon.Velocity = Bot(Instigator.Controller).AdjustToss(EHTransBeacon.Speed, EHTransBeacon.Location, TTargetLoc,false);
		EHTransBeacon.SetTranslocationTarget(Bot(Instigator.Controller).RealTranslocationTarget);
	}
}

function class<DamageType> GetDamageType()
{
	return class'DamTypeEHTelefrag';
}
...
function float GetAIRating()
{
	...
	{
		if ( (EHTransBeacon != None) && EHTransBeacon.IsMonitoring(EHTransbeacon.TranslocationTarget) )
		{
			if ( (GameObject(B.Focus) != None) && (B.Focus != EHTransbeacon.TranslocationTarget) )
			{
				if ( Instigator.Weapon == self )
					SetTimer(0.2,false);
				return 4;
			}
			if  ( (EHTransbeacon.TranslocationTarget == Instigator.Controller.MoveTarget)
					|| (EHTransbeacon.TranslocationTarget == Instigator.Controller.RouteGoal)
					|| (EHTransbeacon.TranslocationTarget == Instigator.Controller.RouteCache[0])
					|| (EHTransbeacon.TranslocationTarget == Instigator.Controller.RouteCache[1])
					|| (EHTransbeacon.TranslocationTarget == Instigator.Controller.RouteCache[2])
					|| (EHTransbeacon.TranslocationTarget == Instigator.Controller.RouteCache[3]) )
				return 4;
		}
...
{
    if ( (PlayerController(Instigator.Controller) != None) && (PlayerController(Instigator.Controller).ViewTarget == EHTransBeacon) )
    {
        ...
        if ( EHTransBeacon != None )
        {
			EHTransbeacon.SetRotation(PlayerController(Instigator.Controller).Rotation);
			EHTransbeacon.SoundVolume = EHTransbeacon.default.SoundVolume;
		}
    }
}
...
simulated function ViewCamera()
{
    if ( EHTransBeacon!=None )
    {
        ...
        {
            PlayerController(Instigator.Controller).SetViewTarget(EHTransBeacon);
            PlayerController(Instigator.Controller).ClientSetViewTarget(EHTransBeacon);
            PlayerController(Instigator.Controller).SetRotation( EHTransBeacon.Rotation );
			EHTransbeacon.SoundVolume = ViewBeaconVolume;
        }
    }
}
...
{
	if ( (EHTransBeacon == None) || (Instigator.Controller.IsA('PlayerController') && (PlayerController(Instigator.Controller).ViewTarget == EHTransBeacon)) )
        ViewPlayer();
    else
        ViewCamera();
}
...
simulated event RenderOverlays( Canvas Canvas )
{
	...
	if ( (PlayerController(Instigator.Controller) != None) && (PlayerController(Instigator.Controller).ViewTarget == EHTransBeacon) )
    {
		...
		{
			dist = VSize(EHTransBeacon.Location - Instigator.Location);
			if ( dist > MaxCamDist )
			{
				clr = 255.0;
			}
    ...
    else
    {
		...
		if ( !bTeamSet && (Instigator.PlayerReplicationInfo != None) && (Instigator.PlayerReplicationInfo.Team != None) )
		{
			...
			if ( Instigator.PlayerReplicationInfo.Team.TeamIndex == 1 )
				Skins[1] = Material'mm_EHWeapons.EHWeaponSkinsB.EHNEWTranslocatorBlue';
		}
...
defaultproperties
{
     ...
     FireModeClass(0)=Class'mm_EHWeapons.EHTransFire'
     FireModeClass(1)=Class'mm_EHWeapons.EHTransRecall'
     PutDownAnim="PutDown"
     ...
     InventoryGroup=10
     PickupClass=Class'mm_EHWeapons.EHTranspickup'
     PlayerViewOffset=(X=28.500000,Y=12.000000,Z=-12.000000)
     PlayerViewPivot=(Pitch=1000,Yaw=400)
     BobDamping=1.800000
     AttachmentClass=Class'mm_EHWeapons.EHTransAttachment'
     ...
     Skins(1)=Texture'mm_EHWeapons.EHWeaponSkinsB.EHNEWTranslocatorTEX'
     Skins(2)=Texture'mm_EHWeapons.EHWeaponSkinsB.EHNEWTranslocatorPUCK'
